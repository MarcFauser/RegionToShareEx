...
using RegionToShareEx.Properties;
using static RegionToShareEx.NativeMethods;
using static RegionToShareEx.ExtensionMethods;
...
public partial class MainWindow
{
    ...
    private readonly DispatcherThrottle _updateSizeAndPos;

    public MainWindow()
    {
        InitializeComponent();

        _updateSizeAndPos = new DispatcherThrottle(DispatcherPriority.Normal, UpdateSizeAndPosCore);
    ...
    private ICollection<string> LoadResolutions()
    {
        ...
        try
        {
            var userDataDirPath = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData), @"RegionToShareEx");
    ...
    protected override void OnSourceInitialized(EventArgs e)
    {
        ...
        var separationLayerWindow = new Window()
        {
            ...
            Title = "Region to Share Ex - Separation Layer",
            ShowInTaskbar = false,
            Top = Top,
            Left = Left,
            Width = 10,
    ...
    public static bool ValidateSettings()
    {
        ...
        {
            var inner = ex.ExceptionChain().OfType<ConfigurationException>().FirstOrDefault(item => !string.IsNullOrEmpty(item.Filename));
            if (inner == null)
                throw;
    ...
    private void UpdateSizeAndPosCore()
    {
